...
# Alias
alias qs='qsub -cwd -V -j y -b y -M "veen@nordicsemi.no" -m e -N `~/usr/bin/gridname.sh $*` -o \$JOB_ID.log $*'
alias qr='qrsh -cwd -V -now no -pty yes $*'
#alias svnnojenkins='svn ci --with-revprop "nojenkins" $*'
...
alias ns='background konsole --workdir `pwd` -e bash'
