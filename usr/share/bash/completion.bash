...
if ! declare -f _fzf_compgen_dir > /dev/null; then
  ...
  _fzf_compgen_dir() {
    command find -L "$1" \
      -name dft -prune -o -name externals -prune -o -name .git -prune -o -name .svn -prune -o -type d \
      -a -not -path "$1" -print 2> /dev/null | sed 's@^\./@@'
  }
  #_fzf_compgen_dir() {
  #  command ag --hidden -g "$1" \
  #    --path-to-ignore ~/.agignore -f -l --dirs=only
  #}
fi
...
d_cmds="${FZF_COMPLETION_DIR_COMMANDS:-fds pushd rmdir}"
a_cmds="
  awk cat diff diff3
